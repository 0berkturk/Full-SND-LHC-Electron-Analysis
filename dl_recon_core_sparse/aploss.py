...
import config
class BinaryAPLoss(torch.autograd.Function):
    @staticmethod
    def forward(ctx, logits, targets, delta=2.):
        delta=config.delta
        classification_grads = torch.zeros(logits.shape).cuda()
        cls_loss = torch.zeros(0).cuda()
        if torch.max(targets) <= 0:  ## if there are no positive sample at all,
            print("error",cls_loss,targets)
            return 0, None, None
...
class APLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        return BinaryAPLoss.apply(logits, targets)

...
class SNDSparseDataset(Dataset):
    def __init__(self,label_fname,perc=None):
        ...
        for prefix in global_prefixes:
            ...
            if hits_key in data:
                vals = data[hits_key]
                cut_mask &= (vals >= getattr(config, f"cut_min_{hits_key}", 0))
            ...
            if qdc_key in data:
                vals = data[qdc_key]
                cut_mask &= (vals >= getattr(config, f"cut_min_{qdc_key}", 0))
        ...
        for prefix, n_layers in layer_configs:
            ...
            if hits_key in data:
                ...
                min_limits = getattr(config, f"cut_min_{hits_key}", [0] * n_layers)
            ...
            if qdc_key in data:
                ...
                min_limits = getattr(config, f"cut_min_{qdc_key}", [0] * n_layers)
    ...
    def __getitem__(self, idx):
        ...
        if "MC" in self.path:
            ...
            qdc_threshold_value_scifi = getattr(config,"qdc_threshold_value_scifi_mc",0)
            ...
            qdc_threshold_value_scifi = getattr(config,"qdc_threshold_value_scifi_data",0)

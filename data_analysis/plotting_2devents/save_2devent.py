...
parser.add_argument("-p", "--path", dest="path", help="path to data file",required=False,default="/eos/experiment/sndlhc/convertedData/physics/2025/") #os.environ["EOSSHIP"]+
parser.add_argument("-f", "--inputFile", dest="inputFile", help="input file data and MC",default="",required=False)
parser.add_argument("-g", "--geoFile", dest="geoFile", help="geofile", default=os.environ["EOSSHIP"]+"/eos/experiment/sndlhc/convertedData/physics/2025/geofile_sndlhc_TI18_V8_2025.root")
...
if options.inputFile=="":
   file_name = f"sndsw_raw-{options.event_number//1000000:04d}.root"
   run_str = str(options.runNumber).zfill(6) # İSMİ DEĞİŞTİRDİK
   print(f"Event: {options.event_number} -> Dosya: {file_name}")
   if YEAR==2023:
      f=ROOT.TFile.Open(f"{options.path}run_{run_str}/{file_name}")
   elif (YEAR==2024 or YEAR==2025):
      print(f"{options.path}*/run_{run_str}/{file_name}*")
      filename = glob.glob(f"{options.path}*/run_{run_str}/{file_name}*")[0]
      print(filename)
...
def drawInfo(pad, k, run, event, timestamp,moreEventInfo=[], darkMode=False):
   ...
   if k == 2:
      ...
      textDL.DrawLatex(0.40, 0.85, "tan#theta_{X} (Step.) X: %.3f" % options.av_step_theta_x)
      textDL.DrawLatex(0.40, 0.70, "tan#theta_{Y} (Step.) Y: %.3f" % options.av_step_theta_y)

      textDL.DrawLatex(0.40, 0.55, "tan#theta_{X} (Lin.) X: %.3f" % options.av_lin_theta_x)
      textDL.DrawLatex(0.40, 0.40, "tan#theta_{Y} (Lin.) Y: %.3f" % options.av_lin_theta_y)

      textDL.DrawLatex(0.40, 0.25, "tan#theta_{X} (UNW.) X: %.3f" % options.unweighted_theta_x)
      textDL.DrawLatex(0.40, 0.10, "tan#theta_{Y} (UNW.) Y: %.3f" % options.unweighted_theta_y)
...
def drawFixedShowerAxis(pad, k, shower_start, ref_point, shower_direction):
   pass
   """line_name= "shower_dir_fixed_" + str(k) # İSİM ÇAKIŞMASIN DİYE fixed EKLENDİ
   if shower_start<100:
      zpos_name='scifi'+str(shower_start//10)
   else:
      zpos_name='us'+str(shower_start//100)
   h[line_name] = ROOT.TLine(h[zpos_name+'_z'],
                             ...
                             h['zmax'],
                             ref_point+shower_direction*h['zmax'])
   h[line_name].SetLineColor(ROOT.kCyan) 
   h[line_name].SetLineStyle(1)
   h[line_name].SetLineWidth(4)
   pad.cd(k)
   h[line_name].Draw()"""

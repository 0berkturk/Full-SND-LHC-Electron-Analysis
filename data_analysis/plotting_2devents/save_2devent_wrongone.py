...
parser.add_argument("-p", "--path", dest="path", help="path to data file",required=False,default=os.environ["EOSSHIP"]+"/eos/experiment/sndlhc/convertedData/physics/2023/")
parser.add_argument("-f", "--inputFile", dest="inputFile", help="input file data and MC",default="",required=False)
parser.add_argument("-g", "--geoFile", dest="geoFile", help="geofile", default=os.environ["EOSSHIP"]+"/eos/experiment/sndlhc/convertedData/physics/2023/geofile_sndlhc_TI18_V3_2023.root")
...
parser.add_argument("-en", "--event_number", dest="event_number", help="event number", type=int,required=False)
...
parser.add_argument("--dl_100", dest="dl_100", type=float, default=0.0, help="DL 100GeV value")
parser.add_argument("--dl_400", dest="dl_400", type=float, default=0.0, help="DL 400GeV value")
parser.add_argument("--qdc_en", dest="qdc_en", type=float, default=0.0, help="QDC Energy value")
parser.add_argument("--dl_cls", dest="dl_cls", type=float, default=0, help="DL CLS value")
parser.add_argument("--frac_qdc", dest="frac_qdc", type=float, default=0.0, help="Fraction QDC value")
parser.add_argument("--frac_hit", dest="frac_hit", type=float, default=0.0, help="Fraction Hit value")


parser.add_argument("--average_step_theta_x", dest="av_step_theta_x", type=float, default=None, help="Shower direction slope dx/dz (x/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--average_step_theta_y", dest="av_step_theta_y", type=float, default=None, help="Shower direction slope dy/dz (y/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--average_lin_theta_x", dest="av_lin_theta_x", type=float, default=None, help="Shower direction slope dx/dz (x/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--average_lin_theta_y", dest="av_lin_theta_y", type=float, default=None, help="Shower direction slope dy/dz (y/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--unweighted_theta_x", dest="unweighted_theta_x", type=float, default=None, help="Shower direction slope dx/dz (x/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--unweighted_theta_y", dest="unweighted_theta_y", type=float, default=None, help="Shower direction slope dy/dz (y/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--neutrino_theta_x", dest="neutrino_theta_x", type=float, default=None, help="Shower direction slope dx/dz (x/z), drawn as an arrow from the highest-QDC SciFi hit")
parser.add_argument("--neutrino_theta_y", dest="neutrino_theta_y", type=float, default=None, help="Shower direction slope dy/dz (y/z), drawn as an arrow from the highest-QDC SciFi hit")

parser.add_argument("--which_angle", dest="which_angle", type=str, default=None, help="Shower direction slope dy/dz (y/z), drawn as an arrow from the highest-QDC SciFi hit")
options = parser.parse_args()
if options.which_angle == "average_step_theta":
    theta_x = options.av_step_theta_x
    theta_y = options.av_step_theta_y
elif options.which_angle == "average_lin_theta":
    theta_x = options.av_lin_theta_x
    theta_y = options.av_lin_theta_y
elif options.which_angle == "unweighted_theta":
    theta_x = options.unweighted_theta_x
    theta_y = options.unweighted_theta_y
elif options.which_angle == "neutrino_theta":
    theta_x = options.neutrino_theta_x 
    theta_y = options.neutrino_theta_y
else:
    theta_x, theta_y = None, None

setattr(options, 'theta_x', theta_x)
setattr(options, 'theta_y', theta_y)
print(f"Seçilen Theta X: {options.theta_x}")
...
if options.inputFile=="":
   file_number = options.event_number//1000000
   file_name = f"sndsw_raw-{file_number:04d}.root"
   print(f"Event: {options.event_number} -> Dosya: {file_name}")
   f=ROOT.TFile.Open(options.path+'run_'+str(options.runNumber).zfill(6)+"/"+file_name)
...
def findMaxQDCScifiHit(event):
       """Scans Digi_ScifiHits and returns the global (x,y,z) position of the SciFi
       hit with the highest summed QDC (over its SiPM channels), or None if no
       valid hit is found.

       Returns:
           (x, y, z) tuple in global coordinates, or None
       """
       bestQDC = None
       bestPos = None
       if not event.FindBranch("Digi_ScifiHits"):
              return None
       for digi in event.Digi_ScifiHits:
              ...
              for side in range(ns):
                     for m in range(digi.GetnSiPMs()):
                            ...
                            if not qdc < 0:
                                   this_qdc += qdc
              if bestQDC is None or this_qdc > bestQDC:
...
def drawShowerAxisFromTheta(pad, k, startPoint, thetaX, thetaY):
       """Draws an arrow (TArrow) representing the shower direction, starting from
       startPoint=(x,y,z) (typically the highest-QDC SciFi hit) and following the
       slope theta_x=dx/dz (for the X projection, k==1) or theta_y=dy/dz (for the
       Y projection, k==2), out to the edge of the current z-range (h['zmax']).

       Mirrors the style of drawShowerAxis (same colour/linestyle), but uses
       TArrow so the direction is visually unambiguous.
       """
       x0, y0, z0 = startPoint
       zEnd = h['zmax']
       if k == 1:
              ...
              pos0 = y0
              slope = thetaY
       posEnd = pos0 + slope*(370 - 275)

       arrow_name = "shower_dir_theta_" + str(k)
       h[arrow_name] = ROOT.TArrow(z0, pos0, zEnd, posEnd, 0.02, "|>")
       h[arrow_name].SetLineColor(ROOT.kCyan)
       h[arrow_name].SetFillColor(ROOT.kCyan)
       h[arrow_name].SetLineStyle(3)
       h[arrow_name].SetLineWidth(3)
       h[arrow_name].SetAngle(30)
       pad.cd(k)
       h[arrow_name].Draw()
...
def loopEvents(
 ...
 for N in range(s,e):
    ...
    # draw user-supplied shower direction (theta_x = dx/dz, theta_y = dy/dz) as an
    # arrow starting from the highest-QDC SciFi hit in this event
    if options.theta_x is not None and options.theta_y is not None:
       maxQDCpos = findMaxQDCScifiHit(event)
       if maxQDCpos is None:
          print("theta_x/theta_y given but no SciFi hit found in this event; skipping arrow")
       else:
          for k in proj:
             drawShowerAxisFromTheta(h['simpleDisplay'], k, maxQDCpos, options.theta_x, options.theta_y)
       h['simpleDisplay'].Update()
...
def drawInfo(pad, k, run, event, timestamp,moreEventInfo=[], darkMode=False):
   ...
   if drawText:
    if k==1 or len(moreEventInfo)<5:
      ...
      pad.cd(k)
   if k == 2:
      padDL = ROOT.TPad("dl_info_"+str(k), "dl_info_"+str(k), 0.70, 0.10, 0.99, 0.35)
      padDL.SetFillStyle(4000)
      padDL.Draw()
      padDL.cd()
      
      textDL = ROOT.TLatex()
      textDL.SetTextAlign(11)
      textDL.SetTextFont(42)
      textDL.SetTextSize(0.08)
      if darkMode: textDL.SetTextColor(ROOT.kWhite)

      val_dl_100 = options.dl_100
      val_dl_400 = options.dl_400
      val_qdc_en = options.qdc_en
      val_dl_cls = options.dl_cls
      val_frac_qdc = options.frac_qdc
      val_frac_hit = options.frac_hit

      textDL.DrawLatex(0.05, 0.85, "DL(100GeV): {:.2f} GeV".format(val_dl_100))
      textDL.DrawLatex(0.05, 0.70, "DL(400GeV): {:.2f} GeV".format(val_dl_400))
      textDL.DrawLatex(0.05, 0.55, "QDC Energy: {:.2f} GeV".format(val_qdc_en))
      textDL.DrawLatex(0.05, 0.40, "DL(CLS): {}".format(val_dl_cls))
      textDL.DrawLatex(0.05, 0.25, "Frac QDC: {:.2f}".format(val_frac_qdc))
      textDL.DrawLatex(0.05, 0.10, "Frac Hit: {:.2f}".format(val_frac_hit)) 

      # Mevcut DL verilerinin altına theta değerlerini yazdırmak için
      # y koordinatlarını daha aşağıdan başlatarak devam edelim:

      textDL.DrawLatex(0.40, 0.85, "Step Theta X: {:.3f}".format(options.av_step_theta_x))
      textDL.DrawLatex(0.40, 0.70, "Step Theta Y: {:.3f}".format(options.av_step_theta_y))

      textDL.DrawLatex(0.40, 0.55, "Lin Theta X: {:.3f}".format(options.av_lin_theta_x))
      textDL.DrawLatex(0.40, 0.40, "Lin Theta Y: {:.3f}".format(options.av_lin_theta_y))

      textDL.DrawLatex(0.40, 0.25, "Unweighted Theta X: {:.3f}".format(options.unweighted_theta_x))
      textDL.DrawLatex(0.40, 0.10, "Unweighted Theta Y: {:.3f}".format(options.unweighted_theta_y)) 
      #textDL.DrawLatex(0.60, 1, "Incoming Neutrino Theta X: {:.6f}".format(options.neutrino_theta_x))
      #textDL.DrawLatex(0.60, 0.80, "Incoming Neutrino Theta Y: {:.6f}".format(options.neutrino_theta_y))    
      pad.cd(k)
...
def drawShowerAxis(pad, k, shower_start, ref_point, shower_direction):
    ...
    old_slope = shower_direction 
    
    if k == 1:
        new_slope = options.unweighted_theta_x
    else:
        new_slope = options.unweighted_theta_y
        
    # Pivot noktası: Duşun başladığı Z konumu
    z_start = h[zpos_name+'_z']
    
    # Orijinal çizginin duş başlangıcındaki X (veya Y) konumu
    pos_start = ref_point + old_slope * z_start
    
    # Yeni çizginin formülü: pos = pos_start + new_slope * (Z - z_start)
    # Z_start noktasında formül: pos_start + 0 = pos_start (Yani iki çizgi aynı noktadan başlar)
    
    line_name_2 = line_name + "_unweighted"
    h[line_name_2] = ROOT.TLine(z_start,
                                pos_start,
                                h['zmax'],
                                pos_start + new_slope * (h['zmax'] - z_start))

    h[line_name_2].SetLineColor(ROOT.kBlack)
    h[line_name_2].SetLineStyle(1)
    h[line_name_2].SetLineWidth(5)
    
    h[line_name_2].Draw()

if options.rootbatch and options.event_number:
    try:
        # Dosya başına 1 milyon event olduğu gerçeğine dayanarak lokal indeksi direkt hesapla
        local_index = options.event_number % 1000000
        
        print(f"\n[INFO], Global Event: {options.event_number} -> Lokal Index: {local_index}. Çiziliyor...")
        
        # Sadece bu indeksi list olarak veriyoruz ki sadece bunu çizip çıksın
        loopEvents(start=[local_index], save=False, auto=True, hitColour="q", darkMode=False)
        
        # ROOT'un çizimi bitirmesi için küçük bir bekleme ve grafik motorunu güncelleme
        ROOT.gSystem.ProcessEvents()
        
        print("[INFO] İşlem tamamlandı! Resim kaydedildi.")
    except Exception as e:
        print(f"[ERROR] Event işlenirken hata oluştu: {e}")
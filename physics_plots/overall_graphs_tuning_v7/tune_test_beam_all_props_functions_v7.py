...
def plot_multiple_hist(qdc_energy_list,N,xmin,xmax,x_label, title, label_str, outdir,alpha_list,name="EMPTY"):
    ...
    for i in range(len(qdc_energy_list)):
        ...
        mu, std = norm.fit(qdc_energy)
        means.append(mu)
        stds.append(std)
        if N!=None:
            bins = np.linspace(xmin, xmax, N)
            plot_data=plt.hist(qdc_energy,bins=bins,label=label_str[i],alpha=alpha_list[i],histtype='step',density=True )
            plt.xlim(xmin, xmax)
        else:
            plot_data=plt.hist(qdc_energy,bins=20,label=label_str[i],alpha=alpha_list[i],histtype='step',density=True )
        plt.axvline(mu, color=plot_data[2][0].get_edgecolor(), linestyle='dashed')

        # Log scale
    diff_mean = means[0]-means[1]
    diff_std = stds[0]-stds[1]
    plt.yscale('log')
    plt.xlabel(x_label)
    plt.ylabel('Density')
    plt.title(title)
    ...
    plt.close()
    return np.abs(diff_mean) , np.abs(diff_std)
...
def load_as_lists(file_list,N,funct, name_data, label_name, config_feature_name):
    ...
    file_list
    ds1 = SNDSparseDataset([0,file_list[0]], N)  
    dataloader1 = DataLoader(
        ds1, 
        batch_size=N,   # FIX 3: Matched capitalization
        shuffle=False,        # FIX 2: Uses the new function argument
        num_workers=8)

    ds2 = SNDSparseDataset([0,file_list[1]], N)  
    ...
    # 1. Create an empty list before the loops to store all results
    all_results = []

    for qdc_threshold_mc in [0, 1,2,3,4 ,5,10,15,20,30]:
        for qdc_threshold_data in [1,2,3,4,5,10,15,20,30]:

            ds1.update_cut(qdc_threshold_value_scifi=qdc_threshold_data, smear_sigma=0)
            ds2.update_cut(qdc_threshold_value_scifi=qdc_threshold_mc, smear_sigma=0)

            scifi, us, ds, energy  = next(iter(dataloader1))
            scifi_mc, us, ds, energy  = next(iter(dataloader2))
            
            if PLOT_MEAN_QDC_ENERGY:
                scifi_prop = torch.sum(scifi, dim=(1,3))
                scifi_prop_mc = torch.sum(scifi_mc, dim=(1,3))

            elif PLOT_MEAN_HIT_NUMBERS:
                scifi_prop = torch.sum(scifi>0, dim=(1,3))
                scifi_prop_mc = torch.sum(scifi_mc>0, dim=(1,3))

            scifi_list = [scifi_prop, scifi_prop_mc]
            name_end = str(qdc_threshold_mc) + str(qdc_threshold_data)
            total, l1, l2 = funct(scifi_list, name_data, label_name, config_feature_name, name_end)
            
            # 2. Save this specific combination to our list
            scifi_list = [scifi_prop, scifi_prop_mc]
        name_end = str(qdc_threshold_mc) + str(qdc_threshold_data)
        
        total, l1, l2 = funct(scifi_list, name_data, label_name, config_feature_name, name_end)
        
        # Unpack the tuples (mean, std) and save them
        all_results.append({
            'mc_cut': qdc_threshold_mc,
            'data_cut': qdc_threshold_data,
            'total_mean': total[0],
            'total_std': total[1],
            'l1_mean': l1[0],
            'l1_std': l1[1],
    ...
    # Helper function to print the smallest 5 for any given metric
    def print_smallest(results_list, metric_key, title):
        # Sort the list from smallest to largest based on the requested metric
        sorted_results = sorted(results_list, key=lambda x: x[metric_key])
        
        print(f"\n--- {title} ---")
        for i in range(5):
            if i < len(sorted_results):
                res = sorted_results[i]
                print(f"MC Cut: {res['mc_cut']}, Data Cut: {res['data_cut']} | {metric_key}: {res[metric_key]:.4f}")

    # Print lists for L1
    print_smallest(all_results, 'l1_mean', "SMALLEST L1 MEAN")
    print_smallest(all_results, 'l1_std', "SMALLEST L1 STD")

    # Print lists for L2
    print_smallest(all_results, 'l2_mean', "SMALLEST L2 MEAN")
    print_smallest(all_results, 'l2_std', "SMALLEST L2 STD")

    # Print lists for Total
    print_smallest(all_results, 'total_mean', "SMALLEST TOTAL MEAN")
    print_smallest(all_results, 'total_std', "SMALLEST TOTAL STD")

    return 0

def plot_for_everything(scifi_list, name, IS_MC_DATA_TITLE, feature_name,name_end):
    x_min, x_max=0,13000
    y_min, y_max=0,60000
    bins_x, bins_y = 40,40

    N=50000
    scifi_total = [tensor.sum(dim=(1)) for tensor in scifi_list]
    ...
    total_mean_diff, total_std_diff = plot_multiple_hist(scifi_total, None , None, None, xlabel,
                    f"{feature_name} Distribution (All SciFi Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_scifi_"+name+name_end)
    ...
    """for i in range(5):"""
    i = 1
    l2_mean_diff, l2_std_diff = plot_multiple_hist([tensor[:,i] for tensor in scifi_list], None , None, None, xlabel,
                f"SciFi {feature_name} at the Station {i+1} Histogram"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_layer_"+str(i+1)+name+name_end)

    i = 2
    l3_mean_diff, l3_std_diff = plot_multiple_hist([tensor[:,i] for tensor in scifi_list], None , None, None, xlabel,
                f"SciFi {feature_name} at the Station {i+1} Histogram"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_layer_"+str(i+1)+name+name_end)

    return (total_mean_diff, total_std_diff), (l2_mean_diff, l2_std_diff), (l3_mean_diff, l3_std_diff)

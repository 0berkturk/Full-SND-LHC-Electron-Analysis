...
def LOAD_PLOT_2D_SCIFI_HIST(file_list,N):
    scifi_list = []
    us_list = []
    ds_hor_list = []
    ds_ver_list = []
    ...
    for k,fname in enumerate(file_list):
        ...
        scifi_sig, scifi_time  = next(iter(dataloader))
        # empty hits has -999 qdc. empty times has 0. remove them during hit calculations        
...
def plot_for_everything(TEST_DATA_DIR,name, IS_MC_DATA_TITLE, feature_name):
    ...
    for i in range(len(list1)):
        total_energy.append(total_list1[i]+total_list2[i]+total_list3[i])

    
    us_ds_list=[]
    for i in range(len(list1)):
        us_ds_list.append(total_list2[i]+total_list3[i])
    ...
    plot_multiple_hist(total_list3, None , None, None, xlabel,
                    f"{feature_name} Distribution (All DS Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_DS_"+name)
    ...
    # 3. DS Horizontal (total_list3)
    plot_1d_beam_energy_graphs(beam_en_list, total_list3, label_list_en, 
                               "mean_total_DS_" + name, show_ideal=False, 
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}',  
                               title=f"Mean {feature_name} vs. Beam Energy(All DS)" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)

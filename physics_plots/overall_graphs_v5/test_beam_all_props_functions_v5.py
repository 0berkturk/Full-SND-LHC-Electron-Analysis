...
from TB_MCEB_COMP_checking.config_TB_QDC import *
...
def load_as_lists(file_list,N):
    ...
    for k,fname in enumerate(file_list):
        ...
        if PLOT_MEAN_HIT_NUMBERS:
            scifi_prop=data[f"scifi_{TIME_KEY[k]}_hits_per_layer"][cut]
            ...
            scifi_prop=data[f"scifi_{TIME_KEY[k]}_qdc_per_layer"][cut]
            ...
            scifi_prop = torch.cat(data["station_hdw"], dim=0)[cut]
            us_prop=torch.as_tensor(data["max_total_hdw"])[cut]
            ds_hor_prop=torch.as_tensor(data["max_hor_hdw"])[cut]
            ds_ver_prop=torch.as_tensor(data["max_ver_hdw"])[cut]
            print("hdw")
        
        elif PLOT_QDC_FRAC:
            scifi_prop=data[f"scifi_{TIME_KEY}time_qdc_per_layer"][cut]
            us_prop=0
            ds_hor_prop=0
            ds_ver_prop=0
            print("qdc frac")
        elif PLOT_HIT_FRAC:
            scifi_prop=data[f"scifi_{TIME_KEY}time_hits_per_layer"][cut]
            us_prop=0
            ds_hor_prop=data["dsh_notime_qdc_per_layer"][cut]
            ds_ver_prop=data["dsv_notime_qdc_per_layer"][cut]
            print("hit frac")
        ...
        """
        print(scifi_prop.shape,us_prop ,ds_hor_prop, ds_ver_prop ,"\n")
...
def plot_for_everything(TEST_DATA_DIR,name, IS_MC_DATA_TITLE, feature_name):
    x_min, x_max=0,13000
    y_min, y_max=0,60000
    bins_x, bins_y = 40,40
    ...
    list1,list2,list3,list4 = load_as_lists(TEST_DATA_DIR,N)
    total_list1 = [tensor.sum(dim=(1)) for tensor in list1]
    total_list2 =[tensor.sum(dim=(1)) for tensor in list2]
    total_list3 =  [tensor.sum(dim=(1)) for tensor in list3]
    total_list4 =  [tensor.sum(dim=(1)) for tensor in list4]
    ...
    all_ds_list =[]
    for i in range(len(list1)):
        all_ds_list.append(total_list3[i]+total_list4[i])
    ...
    plot_multiple_hist(total_list1, None , None, None, xlabel,
                    f"{feature_name} Distribution (All SciFi Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_scifi_"+name)
    plot_multiple_hist(total_list2, None , None, None, xlabel,
                    f"{feature_name} Distribution (All US Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_US_"+name)
    plot_multiple_hist(total_list3, None , None, None, xlabel,
                    f"{feature_name} Distribution (All DS Hor. Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_DS_HOR_"+name)
    plot_multiple_hist(total_list4, None , None, None, xlabel,
                    f"{feature_name} Distribution (All DS Ver. Stations)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_DS_VER_"+name)
    plot_multiple_hist(us_ds_list, None , None, None, xlabel,
                    f"{feature_name} Distribution (US and DS)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_ALL_scifi_us_ds_"+name)
    plot_multiple_hist(total_energy, None , None, None, xlabel,
                    f"{feature_name} Distribution (SciFi, US, and DS)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_ALL_scifi_us_ds_"+name)
    
    plot_multiple_hist(all_ds_list, None , None, None, xlabel,
                f"{feature_name} Distribution (DS)"+IS_MC_DATA_TITLE, label_list_en, outdir=outdirname,alpha_list=[1,1,1,1,1,1], name="hist_total_all_ds_"+name)
    ...
    plot_1d_beam_energy_graphs(beam_en_list, total_list2, label_list_en, 
                               ...
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}',  
                               title=f"Mean {feature_name} vs. Beam Energy(All US Stations)" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)
    ...
    plot_1d_beam_energy_graphs(beam_en_list, total_list3, label_list_en, 
                               ...
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}',  
                               title=f"Mean {feature_name} vs. Beam Energy(DS Hor.)" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)
    ...
    plot_1d_beam_energy_graphs(beam_en_list, total_list4, label_list_en, 
                               ...
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}', 
                               title=f"Mean {feature_name} vs. Beam Energy(DS Ver.)" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)
    ...
    plot_1d_beam_energy_graphs(beam_en_list, us_ds_list, label_list_en, 
                               ...
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}', 
                               title=f"Mean {feature_name} vs. Beam Energy: US & DS Combined" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)
    ...
    plot_1d_beam_energy_graphs(beam_en_list, total_energy, label_list_en, 
                               ...
                               xlabel=beam_xlabel, ylabel=f'Mean {feature_name}', 
                               title=f"Mean {feature_name} vs. Beam Energy: Global (SciFi, US, and DS)" + IS_MC_DATA_TITLE, 
                               outdir=outdirname)

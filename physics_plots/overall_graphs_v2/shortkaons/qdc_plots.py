...
# Sum over channels, width, and height dimensions (dimensions 1, 2, 3)
    ...
    plt.xlabel('Short Kaon Energy [GeV]')
    plt.ylabel('Average QDC')
    plt.title(f'Average QDC vs Short Kaon Energy')
    plt.legend()
    plt.grid()
    if os.path.exists("muon_en_qdc") is False:
        os.mkdir("muon_en_qdc")
    plt.savefig("muon_en_qdc/"+out_name+"_average_qdc_energy.png", dpi=300)
    plt.clf()
    ...
    plt.title('Total QDC vs Short Kaon Energy')
    plt.grid()
    if os.path.exists("muon_en_qdc") is False:
        os.mkdir("muon_en_qdc")
    plt.savefig("muon_en_qdc/"+out_name+"_qdc_energy.png", dpi=300)
    plt.clf()
    ...
    plt.grid()
    plt.savefig("muon_en_qdc/"+out_name+"_scifi_qdc_energy.png", dpi=300)
    plt.clf()
    ...
    plt.grid()
    plt.savefig("muon_en_qdc/"+out_name+"_us_qdc_energy.png", dpi=300)
    plt.clf()
    ...
    plt.grid()
    plt.savefig("muon_en_qdc/"+out_name+"_ds_qdc_energy.png", dpi=300)
    plt.close()
    return total_qdc
...
TEST_DATA_DIR = ["/eos/user/b/beturk/snd/MonteCarlo/particle_gun_advanced_v1/merge_kshort_gun_0_100/test_combined_kshort_0_100_2025.pt"]
...
label_index = 5 # 0: e, 1: numu, 2: tau, 3: nc
if label_index == 5:
    label_str = "Short Kaon"

...
class TestBeamPlotter:
    ...
    def _generate_legend(self, data_dict, mc_dict):
        """Legend accurately mimicking the visual styling of Data (Solid/Filled) vs MC (Dashed/Open)."""
        legend_elements = [
            Line2D([0], [0], color='k', linestyle='-', marker='s', markerfacecolor='k', markeredgecolor='k', label='Data', markersize=8),
            Line2D([0], [0], color='k', linestyle='--', marker='o', markerfacecolor='w', markeredgecolor='k', markeredgewidth=1.5, label='MC', markersize=8)
        ]
        ...
        # Add color swatches for each base label
    ...
    def plot_mean_vs_energy(self, feature_name, data_dict, mc_dict):
        ...
        for (en, label, color), vals in data_dict.items():
            ...
            plt.errorbar(en, mean_val, yerr=std_val, fmt='-s', color=color, 
                         ...
                         markersize=7, capsize=4, elinewidth=1.5, alpha=0.8)
        ...
        for (en, label, color), vals in mc_dict.items():
            ...
            plt.errorbar(en+3, mean_val, yerr=std_val, fmt='--o', color=color, 
    ...
    def plot_combined_layers_mean(self, features_data, features_mc, metric="QDC", layers=[2, 3, 4]):
        ...
        layer_colors = { 1: '#ff7f0e', 2: '#d62728', 3: '#1f77b4', 4: '#2ca02c', 5: '#9467bd' }
        available_markers = ['o', 's', '^', 'D', 'v', 'p', '*']
        label_markers = {}
        all_labels = set()
        
        # Collect all unique configuration labels present in the chosen layers
        for layer in layers:
            feat_name = f"Station {layer} {metric}"
            if feat_name in features_data:
                all_labels.update([lbl for en, lbl, color in features_data[feat_name].keys()])
            if feat_name in features_mc:
                all_labels.update([lbl for en, lbl, color in features_mc[feat_name].keys()])
                
        # Assign a unique shape to each configuration label
        for i, lbl in enumerate(sorted(list(all_labels))):
            label_markers[lbl] = available_markers[i % len(available_markers)]
        
        for layer in layers:
            feat_name = f"Station {layer} {metric}"
            l_color = layer_colors.get(layer, 'k') # Color is now dictated by the LAYER
            ...
            if feat_name in features_data:
                ...
                for lbl in labels:
                    ...
                    plt.errorbar(ens, means, yerr=stds, fmt=f'-{marker}', color=l_color,
                                 markerfacecolor=l_color, markeredgecolor=l_color,
                                 markersize=7, capsize=4, elinewidth=1.5, alpha=0.85)
            ...
            if feat_name in features_mc:
                ...
                for lbl in labels:
                    ...
                    offset_ens = [e +3 for e in ens]
                    plt.errorbar(offset_ens, means, yerr=stds, fmt=f'--{marker}', color=l_color, 
                                 markerfacecolor='white', markeredgecolor=l_color, markeredgewidth=1.5,
                                 markersize=7, capsize=4, elinewidth=1.5, alpha=0.85)

        plt.xlabel('Beam Energy [GeV]', fontsize=12)
        plt.ylabel(f'Mean {metric}', fontsize=12)
        plt.title(f'Mean {metric} vs. Beam Energy (Station {", ".join(map(str, layers))})', fontsize=14)
        
        # Build Legend matching the Combined Graph logic
        legend_elements = [
            Line2D([0], [0], color='k', linestyle='-', marker='s', markerfacecolor='k', label='Data', markersize=8),
            Line2D([0], [0], color='k', linestyle='--', marker='o', markerfacecolor='w', markeredgecolor='k', markeredgewidth=1.5, label='MC', markersize=8)
        ]
        # Configurations -> Shapes
        for lbl in sorted(list(all_labels)):
            legend_elements.append(Line2D([0], [0], marker=label_markers[lbl], color='w', label=lbl, markerfacecolor='gray', markersize=9))
        # Layers -> Colors
        for layer in layers:
            legend_elements.append(Line2D([0], [0], color=layer_colors.get(layer, 'k'), linestyle='-', linewidth=3, label=f'Station {layer}'))
            ...
            if feat_name in features_data:
                ...
                for lbl in labels:
                    ...
                    plt.errorbar(ens, [p[0] for p in peaks_and_errs], yerr=[p[1] for p in peaks_and_errs], 
                                 fmt=f'-{marker}', color=l_color, markerfacecolor=l_color, markeredgecolor=l_color, 
                                 markersize=7, capsize=4, elinewidth=1.5, alpha=0.85)
            ...
            if feat_name in features_mc:
                ...
                for lbl in labels:
                    ...
                    plt.errorbar(offset_ens, [p[0] for p in peaks_and_errs], yerr=[p[1] for p in peaks_and_errs], 
                                 fmt=f'--{marker}', color=l_color, markerfacecolor='white', markeredgecolor=l_color, markeredgewidth=1.5,
                                 markersize=7, capsize=4, elinewidth=1.5, alpha=0.85)

        plt.xlabel('Beam Energy [GeV]', fontsize=12)
        plt.ylabel(f'Highest Bin {metric}', fontsize=12)
        plt.title(f'Highest Bin (Peak) {metric} vs. Beam Energy (Station {", ".join(map(str, layers))})', fontsize=14)
                ...
                for t_win_data in dict_cuts.get("t_window_data", [(2.3, 0.5)]):
                    for t_win_mc in dict_cuts.get("t_window_mc", [(5, 5)]):
                        for noise_sigma in dict_cuts.get("noise_sigma", [0]):
                            for q_max in dict_cuts.get("q_max", [config.q_max]):
                                print(q_max)
                ...
                for threshold in thresholds:
                    for i in range(len(time_window_max)):
                        ...
                        if plane==1:
                            ...
                            plane_t="Y"
                        title_suffix = f"({energy_str} {base_label} | Station {layer+1} Plane {plane_t})"
